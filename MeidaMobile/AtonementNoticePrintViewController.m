...
@interface AtonementNoticePrintViewController ()<DatetimePickerHandler>
    ...
    if (matchLaws) {
        ...
        if (matchInfo) {
            ...
            if ([matchInfo objectForKey:@"matchLaw"]) {
                ...
                if([matchStr containsString:@"第"]){
                    NSRange range = [matchStr rangeOfString:@"第"];
                    matchStr = [matchStr substringFromIndex:range.location];
                    matchStr = [matchStr stringByReplacingOccurrencesOfString:@"、" withString:@"规定，根据"];
                }
            }
